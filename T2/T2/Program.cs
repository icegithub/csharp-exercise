...
namespace T2
{
    public class Student
    {
        ...
        {
            if (num.Length != numLength || !Regex.IsMatch(num, "\\d{9}"))
        ...
        public String NUM
        {
            ...
            set
            {
                if (value.Length != numLength || !Regex.IsMatch(value, "\\d{9}"))
                    throw (new System.Exception());
                this.num = value;
            }
        }
    ...
    public class Classroom
    {
        ...
        public bool sortByNum()
        {
            ...
            for(i=0;i<studentNum;i++)
            {
                for (j = i; j < studentNum; j++)
                    if (!numCompare(studList[j].NUM,studList[i].NUM))
                {
                    stuTmp=studList[i];
        ...
        private bool numCompare(String s1,String s2)
        {
            int i;
            for (i = 0; i < 9; i++)
            {
                if (Convert.ToInt32(s1[i]) > Convert.ToInt32(s2[i])) return true;
            }
            return false;
        }
    }
